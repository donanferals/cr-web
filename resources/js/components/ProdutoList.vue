...
<script setup>
  ...
  const carregarProdutos = async () => {
    loading.value = true;
    try {
      let response = await fetch(`http://localhost:8000/api/produtos?query=${query.value}&page=${page.value}&per_page=${perPage.value}&tipo=${tipo.value}`);
